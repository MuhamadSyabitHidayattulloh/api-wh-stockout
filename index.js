...
app.use('/api-docs', basicAuth, swaggerUi.serve, swaggerUi.setup(specs, {
...
app.get('/api-docs/export/:format', basicAuth, (req, res) => {
  ...
  if (format === 'json') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="warehouse-stockout-api.json"');
    res.json(specs);
  } else if (format === 'yaml') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="warehouse-stockout-api.yaml"');
    res.send(yaml);
  } else {
    res.status(400).json({ 
